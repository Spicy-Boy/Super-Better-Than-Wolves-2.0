...
public class FCEntityLightningBolt extends EntityWeatherEffect
{
	...
	public FCEntityLightningBolt( World world, double dPosX, double dPosY, double dPosZ )
	{
				...
				MathHelper.floor_double( dPosZ ), 10 ) )
		{
			...
			//AARON modifies this vvv if statement to make lightning fire configurable
//			if ( !IsStrikingLightningRod() )
			if (/*AARON ADDED*/ SuperBTW.instance.getLightningFireEnabled() && !IsStrikingLightningRod())
			{
	...
	public void onUpdate()
	{
		...
		else if ( m_iLightningState < 0 )
		{
			...
			else if ( m_iLightningState < -rand.nextInt( 10 ) )
			{
				...
				//AARON modifies the if statement below to make fire configurable!
				if ( /*AARON ADDED*/ SuperBTW.instance.getLightningFireEnabled() &&
						!worldObj.isRemote && !IsStrikingLightningRod() && 
	...
	{
		//AARON added this escape hatch to prevent log burning when lightning fire is disabled
		if (!SuperBTW.instance.getLightningFireEnabled())
		{
			return;
		}
