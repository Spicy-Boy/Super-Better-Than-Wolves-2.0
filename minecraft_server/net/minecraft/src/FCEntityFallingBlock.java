...
public class FCEntityFallingBlock extends EntityFallingSand
{
    ...
    private boolean AttemptToReplaceBlockAtPosition( int i, int j, int k )
    {
    		...
    		!Block.blocksList[this.blockID].CanFallIntoBlockAtPos(this.worldObj, i, j - 1, k ) )
    	{
        	...
        	if ( destBlock != null )
        	{
        		//if this block is specified to not crush weak blocks or destination block is a plant, do not crush!
        		if ( ( !SuperBTWDefinitions.doesBlockCrushWeaklings(this.blockID) )
        				|| (SuperBTWDefinitions.isWeakBlock(this.blockID) && SuperBTWDefinitions.isWeakBlock(destBlock.blockID)) )
        		{
        			//TESTER VVV
//        			System.out.println("NOT CRUSHED!");
        			return false;
        		}
        		else if (SuperBTWDefinitions.isWeakBlock(this.blockID) && SuperBTWDefinitions.isPlantBlock(destBlock.blockID))
        		{
        			//TESTER VVV
//        			System.out.println("PLANT CRUSHED BY WEAKLING!");
