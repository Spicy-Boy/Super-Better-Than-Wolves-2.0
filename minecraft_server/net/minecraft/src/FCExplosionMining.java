...
public class FCExplosionMining
{
        ...
        for(int k2 = 0; k2 < list.size(); k2++)
        {
            ...
            if(d4 <= 1.0D)
            {
                ...
                if ( entity instanceof EntityItem )
                {
                	...
                	//AARON changed to include modern blocks and items
                	if ( iItemID != Item.coal.itemID &&
            			...
            			iItemID != FCBetterThanWolves.fcItemChunkGoldOre.itemID &&
            			iItemID != FCBetterThanWolves.fcItemChunkIronOre.itemID &&
            			iItemID != FCBetterThanWolves.fcBlockCobblestoneLoose.blockID &&
            			iItemID != Item.diamond.itemID
                		)
                	{
    ...
    private void DestroyBlock( int i, int j, int k )
    {
        ...
        if ( iBlockID > 0 )
        {
        	...
        	//AARON CHANGED to convert stone to loose cobblestone
        	if ( iBlockID == Block.stone.blockID  )
        	{
        		// convert stone to cobblestone
        		destroyedBlock.dropBlockAsItem_do( worldObj, i, j, k, new ItemStack( FCBetterThanWolves.fcBlockCobblestoneLoose, 1, 0 ) );
        	}
        	else if ( iBlockID == Block.cobblestone.blockID  )
        	{
