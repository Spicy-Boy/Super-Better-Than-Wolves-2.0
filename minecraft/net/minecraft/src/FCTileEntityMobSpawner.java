        ...
        if ( !worldObj.isRemote )
        {
	    	...
	    	if ( worldObj.rand.nextInt( 1200 ) == 0 ) // about once a minute
	    	{
	            if ( worldObj.checkChunksExist( xCoord - 4, yCoord - 1, zCoord - 4, xCoord + 4, yCoord + 4, zCoord + 4 ) )
	            {
		    		...
		    		//AARON made it loose cobble too!
		    		if ( worldObj.getBlockId( targetI, targetJ, targetK ) == Block.cobblestone.blockID || worldObj.getBlockId( targetI, targetJ, targetK ) == FCBetterThanWolves.fcBlockCobblestoneLoose.blockID)
		    		{
